...
def _parse_arcade_max_scores_line(line: str) -> tuple[str, str, int, int] | None:
    """Return chart key, display name, level, and critical max score from one CSV row."""
    parts = line.rsplit(",", 3)
    if len(parts) != 4:
        return None
    ...
    if not display_name:
        return None
    return chart_key, display_name, int(level_str), int(critical_str)


@lru_cache(maxsize=4)
def _load_arcade_max_scores_data(
    path_str: str,
    ...
    for line in Path(path_str).read_text(encoding="utf-8").splitlines():
        ...
        if parsed is None:
            continue
        chart_key, display_name, level, critical_max = parsed
        max_scores[chart_key] = critical_max
        levels[chart_key] = level
        song_id = chart_key.rsplit("/", 1)[0]
        display_names.setdefault(song_id, display_name)
    return max_scores, display_names, levels
...
def load_arcade_chart_levels(path: Path) -> dict[str, int]:
    """Map chart keys (Song/Difficulty) to rating level (CSV level column)."""
    return _load_arcade_max_scores_data(str(path))[2]
